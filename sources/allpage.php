<?php
    ...
    $banner_hotro = $d->rawQueryOne("select photo from #_photo where type = ? and act = ? limit 0,1",array('banner-hotro','photo_static'));
    ...
    $splistmenu = $d->rawQuery("select ten$lang, tenkhongdauvi, tenkhongdauen, id from #_product_list where type = ? and menu > 0 and hienthi > 0 order by stt,id desc",array('san-pham'));
    $splistindex = $d->rawQuery("select ten$lang, tenkhongdauvi, tenkhongdauen, id from #_product_list where type = ? and hienthi > 0 order by stt,id desc",array('san-pham'));
    ...
    $ht = $d->rawQuery("select ten$lang, tenkhongdauvi, tenkhongdauen, info_more from #_news where type = ? and hienthi > 0 order by stt,id desc",array('ho-tro'));
