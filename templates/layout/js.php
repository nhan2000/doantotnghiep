...
<?php
    ...
    $js->setJs("./admin/assets/js/priceFormat.js");
    ...
    <script type="text/javascript">
        grecaptcha.ready(function () {
           if($("#recaptchaResponseNewsletter").exists()){
            ...
            <?php if($source=='kygui') { ?>
                grecaptcha.execute('<?=$config['googleAPI']['recaptcha']['sitekey']?>', { action: 'kygui' }).then(function (token) {
                    var recaptchaResponsekygui = document.getElementById('recaptchaResponsekygui');
                    recaptchaResponsekygui.value = token;
                });
            <?php } ?>
