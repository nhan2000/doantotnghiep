<?php
	...
	function save_permission_group()
	{
		...
		if($id)
		{
			...
			if(!$row['id']) $func->transfer("Nhóm quyền này không tồn tại", "index.php?com=user&act=permission_group&p=".$curPage, false);
	...
	function save_item_admin()
	{
		...
		if($id)
		{
			if($func->check_permission())
			{
				...
				if($row['id']) $func->transfer("Bạn không có quyền trên tài khoản này. Mọi thắc mắc xin liên hệ quản trị website", "index.php?com=user&act=man_admin&p=".$curPage, false);
			}
		...
		else
		{
			...
			if($row['id']) $func->transfer("Tên đăng nhập nay đã tồn tại. Xin chọn tên khác", "index.php?com=user&act=man_admin&p=".$curPage, false);
	...
	function save_item()
	{
		...
		if($id)
		{
			if($func->check_permission())
			{
				...
				if($row['id']) $func->transfer("Bạn không có quyền trên tài khoản này. Mọi thắc mắc xin liên hệ quản trị website", "index.php?com=user&act=man&p=".$curPage, false);
			}
		...
		else
		{
			$username = isset($data['username']) ? $data['username'] : '';
			$row = $d->rawQueryOne("select id from #_user where username = ? limit 0,1",array($username));
			if($row['id']) $func->transfer("Tên đăng nhập nay đã tồn tại. Xin chọn tên khác", "index.php?com=user&act=edit&id=".$id."&p=".$curPage, false);
	...
	function edit()
	{
		...
		if(!empty($_POST))
		{
			...
			else
			{
				...
				if($row['id']) $func->transfer("Tên đăng nhập này đã tồn tại","index.php?com=user&act=admin_edit", false);
