		...
		if($id)
		{
			...
			{	
				...
				if($photo = $func->uploadImage("file", $config[$com][$type][$dfgallery][$val]['img_type_photo'], "../upload/".$com."/",$file_name))
				{
					...
					$data['photo'] = $photo;
					$func->correctImageOrientation("../upload/".$com."/".$photo);	
			...
			if($numberPhoto && $dataMultiTemp)
			{
				...
				if(isset($flag_fail) && isset($flag_success))
				{
					...
					else if($flag_success > 0)
					{
						...
						{
							$func->transfer("Lưu toàn bộ dữ liệu thành công.", "index.php?com=".$com."&act=man_photo&idc=".$idc."&type=".$type."&kind=".$kind."&val=".$val."&p=".$curPage);
						}
					}
				}
				else
