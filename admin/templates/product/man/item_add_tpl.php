    ...
    <form class="validation-form" novalidate method="post" action="<?=$linkSave?>" enctype="multipart/form-data">
		            		...
		            		<div class="form-group-category row">
							    ...
							    <?php } ?>
							     <?php if(isset($config['product'][$type]['tinh']) && $config['product'][$type]['tinh'] == true) { ?>
							    	<div class="form-group col-xl-6 col-sm-4">
					                    <label class="d-block" for="id_size">Danh mục tỉnh/thành phố:</label>
					                    <?=$func->get_ajax_place("city","Tỉnh/thành phố")?>
					                </div>
							    <?php } ?>
							     <?php if(isset($config['product'][$type]['huyen']) && $config['product'][$type]['huyen'] == true) { ?>
							    	<div class="form-group col-xl-6 col-sm-4">
					                    <label class="d-block" for="id_size">Danh mục Quận/huyện:</label>
			            ...
			            <div class="card-body">
		            		<div class="form-group-category row">
							    <?php if(isset($config['product'][$type]['mod_dientich']) && $config['product'][$type]['mod_dientich'] == true) { ?>
							    	<div class="form-group col-xl-6 col-sm-4">
					                    <label class="d-block" for="">Danh mục diện tích:</label>
					                   	<?= $func->get_ajax_mod('mod_dientich',(isset($item['mod_dientich']) && $item['mod_dientich'] != '') ? $item['mod_dientich'] : 0 ,'Diện tích') ?>
					                </div>
							    <?php } ?>
							    <?php if(isset($config['product'][$type]['mod_huong']) && $config['product'][$type]['mod_huong'] == true) { ?>
							    	<div class="form-group col-xl-6 col-sm-4">
					                    <label class="d-block" for="">Danh mục hướng:</label>
					                   	<?= $func->get_ajax_mod('mod_huong',(isset($item['mod_huong']) && $item['mod_huong'] != '') ? $item['mod_huong'] : 0 ,'Hướng') ?>
					                </div>
							    <?php } ?>
							    <?php if(isset($config['product'][$type]['mod_gia']) && $config['product'][$type]['mod_gia'] == true) { ?>
							    	<div class="form-group col-xl-6 col-sm-4">
					                    <label class="d-block" for="">Danh mục mức giá:</label>
					                   	<?= $func->get_ajax_mod('mod_gia',(isset($item['mod_gia']) && $item['mod_gia'] != '') ? $item['mod_gia'] : 0 ,'Mức giá') ?>
					                </div>
							    <?php } ?>
							</div>
			            </div>
                ...
                <div class="row">
				    ...
				    <?php if(isset($config['product'][$type]['gia_text']) && $config['product'][$type]['gia_text'] == true) { ?>
						<div class="form-group col-md-4 col-sm-6">
							...
							<input type="text" class="form-control" name="data[info_more][gia]" id="gia" placeholder="Giá " value="<?=$info_more['gia']?>">
						</div>
					<?php } ?>
					<?php if(isset($config['product'][$type]['dientich']) && $config['product'][$type]['dientich'] == true) { ?>
						<div class="form-group col-md-4 col-sm-6">
							<label for="dientich">Diện tích:</label>
							<input type="text" class="form-control" name="data[info_more][dientich]" id="dientich" placeholder="Diện tích " value="<?=$info_more['dientich']?>">
						</div>
					<?php } ?>
					<?php if(isset($config['product'][$type]['tinhtrang']) && $config['product'][$type]['tinhtrang'] == true) { ?>
						<div class="form-group col-md-4 col-sm-6">
							<label for="tinhtrang">Tình trạng:</label>
							<input type="text" class="form-control" name="data[info_more][tinhtrang]" id="tinhtrang" placeholder="Tình trạng " value="<?=$info_more['tinhtrang']?>">
						</div>
					<?php } ?>
				</div>
            </div>
        ...
        <?php if($config['user']['active'] == true && $config['user']['visitor'] == true && $item['id_user'] > 0) { ?>
			<div class="card card-primary card-outline text-sm">
	            ...
	            <div class="card-body">
                    <div class="form-group">
                        <label for="ten<?=$k?>">Tài khoản đăng:</label>
                        <?=$func->get_info($item['id_user'],'member','username' )?>
                    </div>

					<div class="form-group">
						<label for="ten">Họ tên:</label>
						<input type="text" class="form-control" name="data[info_user][ten]" id="ten" placeholder="Họ tên " value="<?=$info_user['ten']?>">
					</div>
					<div class="form-group">
					...
					<div class="form-group">
						...
						<input type="text" class="form-control" name="data[info_user][dienthoai]" id="dienthoai" placeholder="Điện thoại" value="<?=$info_user['dienthoai']?>">
					</div>
					<div class="form-group">
						<label for="diachi">Địa chỉ:</label>
						<input type="text" class="form-control" name="data[info_user][diachi]" id="diachi" placeholder="Địa chỉ" value="<?=$info_user['diachi']?>">
					</div>
	            </div>
	        </div>
	    <?php } ?>
