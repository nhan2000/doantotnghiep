...

{
	...
	public function showProduct($v, $col='')

	{

		global $lang;

		$ifm_pr = (isset($v['info_more']) && $v['info_more'] != '') ? json_decode($v['info_more'],true) : null;

		$noimage="this.src='".THUMBS."/280x210x2/assets/images/noimage.png'";
		...
		$rt.='<div class="img_pro img_"><a href="'.$v['tenkhongdau'.$lang].'" title="'.$v['ten'.$lang].'" >';

		$rt.='<img class="img-responsive" alt="'.$v['ten'.$lang].'"  onerror='.$noimage.' src="watermark/product/280x210x1/'.UPLOAD_PRODUCT_L.$v['photo'].'" > ';

		$rt.='</a>';

		if($v['new']==1){$rt.='<img class="img_newbds" src="assets/images/new_022Z.gif">';}

		$rt.='</div>';

		$rt.='<h3 class="name_pro"><a href="'.$v['tenkhongdau'.$lang].'" title="'.$v['ten'.$lang].'" >'.$v['ten'.$lang].'</a></h3>';

		$rt.= '<div class="info_pro flex_row">

		<p>Giá: <span>'. $ifm_pr['gia'] .'</span></p>

		<p>Hướng: <span>'. $this->get_name_huong($v['mod_huong']) .'</span></p>

		<p>Diện tích: <span>'. $ifm_pr['dientich'] .'</span></p>

		<p>Tình trạng: <span>'. $ifm_pr['tinhtrang'] .'</span></p></div>';
	...
	public function get_info($id,$table,$col ='tenvi' )

	{	

		$row = $this->d->rawQueryOne("select $col as info from #_$table where id = ? limit 0,1",array($id));

		return $row['info'];

	}

	public function get_name_huong($id)

	{	

		$row = $this->d->rawQueryOne("select tenvi from #_thuoctinh where id = ? limit 0,1",array($id));

		return $row['tenvi'];

	}

	public function get_gia_pro($gia='', $giamoi='')
	...
	{	
		...
		if(($gia>0) & ($giamoi>0)){
			...
			$rt_price.='<span class="giamoi">'.number_format($giamoi). ' đ</span> <span class="giacu">(' .number_format($gia). ' đ)</span>';

		}else{

			if($gia>0){

				$rt_price.='<span class="giamoi">' .number_format($gia). ' đ</span>';

			}else{

				if($giamoi>0){

					$rt_price.='<span class="giamoi">' .number_format($giamoi). ' đ</span>';

				}else{

					$rt_price.='<a href="lien-he.html" title="'.lienhe.'">'.lienhe.'</a>';
	...
	public function showNews($v, $col='')

	{

		global $lang;

		$noimage="this.src='".THUMBS."/280x200x2/assets/images/noimage.png'";

		$rt='';

		$rt.='<div class="'.$col.'">';

		$rt.='<div class="news_box flex_row">';

		$rt.='<div class="img_news img_"><a href="'.$v['tenkhongdau'.$lang].'" title="'.$v['ten'.$lang].'" >';

		$rt.='<img class="img-responsive" alt="'.$v['ten'.$lang].'"  onerror='.$noimage.' src="'.THUMBS.'/280x200x1/'.UPLOAD_NEWS_L.$v['photo'].'" > ';

		$rt.='</a></div>';

		$rt.='<div class="content_news"><h3 class="name_news"><a href="'.$v['tenkhongdau'.$lang].'" title="'.$v['ten'.$lang].'" >'.$v['ten'.$lang].'</a></h3>';

		$rt.='<div class="date_news">Ngày đăng: '. date("d/m/Y",$v['ngaytao']) .'</div>';

		$rt.='<div class="des_news">'.$v['mota'.$lang].'</div>';
	...
	/* Get thuộc tính by ajax */

	public function get_ajax_mod($type='',$id_mod='',$chosse='Chọn thuộc tính')

	{

		

		$rows = $this->d->rawQuery("select tenvi, id from #_thuoctinh where hienthi=1 and type='".$type."' order by id asc");



		$str = '<select id="'.$type.'" name="data['.$type.']" class="form-control select2"><option value="0">'.$chosse.' </option>';

		foreach($rows as $v)

		{

			if(isset($id_mod) && ($v["id"] == (int)$id_mod)) $selected = "selected";
