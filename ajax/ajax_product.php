<?php 
	...
	/* Get data */
	$sql = "select ten$lang, tenkhongdauvi, tenkhongdauen, id, photo, gia,giamoi,giakm, type,info_more,mod_huong,new from #_product where type='san-pham' $where and noibat > 0 and hienthi > 0 order by stt,id desc";
...
<?php if($countItems) { 
	...
	foreach($items as $k=>$v){ $func->showProduct($v,'col-xs-6 col-sm-4 col-md-4'); }
	echo'</div>';
	?>
